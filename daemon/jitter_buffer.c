...
static int queue_packet(struct media_packet *mp, struct codec_packet *p) {
	...
	ilog(LOG_DEBUG, "Seq %d  buflen %d",ntohs(mp->rtp->seq_num), jb->buffer_len);
...
static void handle_clock_drift(struct media_packet *mp, struct codec_packet *p) {
	...
	uint32_t ts_diff = (uint32_t) ts - (uint32_t) jb->first_send_ts; // allow for wrap-around
	int seq_diff = ntohs(mp->rtp->seq_num) - jb->first_seq;
	unsigned long long ts_diff_us =
		(unsigned long long) (ts_diff)* 1000000 / clockrate;
	struct timeval to_send = jb->first_send;
	timeval_add_usec(&to_send, ts_diff_us);
	long long ts_diff_us1 = timeval_diff(&rtpe_now, &to_send);

	jb->clock_drift_val = ts_diff_us1/seq_diff;
	jb->clock_drift_enable = 0;
	jb->cont_buff_err = 0;
}
