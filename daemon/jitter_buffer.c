...
static int get_clock_rate(struct packet_handler_ctx *phc, int payload_type){
	const struct rtp_payload_type *rtp_pt = NULL;
        ...
        {
                ilog(LOG_INFO, "Clockrate already present =%d", phc->sink->jb.clock_rate);
	...
	if(rtp_pt)
		...
		ilog(LOG_INFO, "buffer_packet clock_rate=%d",rtp_pt->clock_rate); //TODO nodata disable jitterbuffer for this call
       }
        else 
...
static void check_buffered_packets(struct jitter_buffer *jb, unsigned int len){
        ilog(LOG_INFO, "check_buffered_packets len=%d", len);
...
static int queue_packet(struct packet_handler_ctx *phc, struct codec_packet *p){
	...
	ilog(LOG_INFO, "buffer_packet phc->sink->first_send.tv_sec=%lu, usec=%06lu , phc->sink->first_send_ts=%lu",phc->sink->jb.first_send.tv_sec,phc->sink->jb.first_send.tv_usec,phc->sink->jb.first_send_ts);
	...
	ilog(LOG_INFO, "buffer_packet ts_diff=%d,ts_diff_us=%llu",ts_diff,ts_diff_us);
	timeval_add_usec(&p->to_send, ts_diff_us);
	ilog(LOG_INFO, "buffer_packet ts_diff=%d,ts_diff_us=%llu",ts_diff,ts_diff_us);
	struct timeval tv;
	gettimeofday(&tv, NULL);
	ilog(LOG_INFO, "buffer_packet tv_now sec=%ld, usec=%06ld", tv.tv_sec, tv.tv_usec);
...
int buffer_packet(struct packet_handler_ctx *phc) {
        ilog(LOG_INFO, "buffer_packet pthread_self=%lu",pthread_self());
        int ret=0;
        char *buffer;
	...
	str_init_len(&phc->s, buffer, phc->s.len);
		...
		ilog(LOG_INFO, "buffer_packet length of queue =%d", get_queue_length(phc->sink->buffer_timer));
        }
        else
        {
...
{
	ilog(LOG_DEBUG, "set_jitter_values");
        int ret=0;
	if(jb->next_exp_seq)
	{
                mutex_lock(&jb->lock);
